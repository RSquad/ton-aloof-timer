...
test("reset timer", async () => {
  ...
  try {
    ...
    const { getCurrentSessionResultDecoded, getSessionsResultDecoded } = await getTimerClientSessions(
      tonClient,
      timerClientAddress
    );
    expect(getCurrentSessionResultDecoded.value.s.status).toBe("1");
    console.log([getCurrentSessionResultDecoded.value.s, getSessionsResultDecoded.value.ss]);
  } catch (err) {
    throw new Error(err);
  }
});
...
test("wait for timer", async () => {
  try {
    ...
    const session = getSessionsResultDecoded.value.ss.find((o: any) => o.id === "2");
